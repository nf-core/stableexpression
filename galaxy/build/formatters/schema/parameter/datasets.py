...
class DatasetsParameterFormatter(BaseParameterFormatter):
    ...
    SAMPLESHEET_PARAM_NAME: ClassVar[str] = "samplesheet"
    CONDITION_NAME: ClassVar[str] = "datasets"
    ...
    def get_input(self) -> str:
        ...
        ).replace(self.param, self.SAMPLESHEET_PARAM_NAME)
        # changing label
        ...
        return f""" \t\t\t<conditional name="{self.CONDITION_NAME}">
    ...
    def get_cli(self) -> str:
        ...
        return s.replace(
            "SAMPLESHEET",
            f"{self.section}.{self.CONDITION_NAME}.{self.SAMPLESHEET_PARAM_NAME}",
        )
