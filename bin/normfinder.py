...
class StatsExtension:
    ...
    def not_null_values(self):
        return (
            self._expr
            .list
            .eval(pl.element().drop_nulls().drop_nans())
            .list
        )
...
class NormFinder:
    ...
    def __post_init__(self):
        ...
        self.group_to_samples_dict = { d["group"]: d["sample"] for d in group_to_sample_df.to_dicts() }
    ...
    def correct_negative_values(
        ...
        genes_with_negative_values = (
            ...
            .select(col for col in self.genes if
                    (intra_var_df[col] < 0).all())  # intra_var_df has only one row but it is a dataframe
            .columns
        )
    ...
    def get_unbiased_intragroup_variance_for_group(
        ...
        mean_over_genes = group_count_df.mean().transpose().to_series().rename("mean_over_genes_for_sample")
        ...
        sample_variance_df = (
            ...
            .transpose(include_header=True, column_names=self.genes) # columns are now genes
            ...
            .with_columns(
                sample_variance=pl.concat_list(samples).row.sum() / ((ng - 1) * (1 - 2 / self.n_genes)) # sum over j (samples) of r_igj ^2 terms
            )
        ...
        intra_var_df = (
            sample_variance_df
            .select([
                ( pl.col(c) - sample_variance_sum_over_genes / (self.n_genes * (self.n_genes -1)) ).alias(c)
                for c in self.genes
            ])
        )
        # if some values are negative, we need a special process
        corrected_intra_var_df = self.correct_negative_values(intra_var_df, group_count_df)
    ...
    def get_unbiased_intragroup_variances(self):
        ...
        for group, samples in tqdm(self.group_to_samples_dict.items()):
            ...
            group_unbiased_intragroup_variance_df = self.get_unbiased_intragroup_variance_for_group(
                chunk_df,
                means_over_samples_df,
                group_overall_mean,
                samples
            )

            # storing intragroup values for each gene in this group
            unbiased_intragroup_variance_dfs.append(group_unbiased_intragroup_variance_df)
    ...
    def adjust_for_nb_of_samples_in_groups(self, unbiased_intragroup_variance_df):
        n_samples_list = [ len(samples) for samples in self.group_to_samples_dict.values() ]
        return (
    ...
    def get_unbiased_intergroup_variance(self, gene_means_in_groups_df, dataset_overall_mean):
        ...
        return (
            ...
            .select([ (pl.col(c) ** 2).alias(c) for c in self.genes ]) # square to get variance
        )
    ...
    def compute_gamma_factor(self, diff_df, vardiff_df):
        ...
        first_term = (
            ...
            .sum() # sum over rows
            .select(
                ( pl.col("sum_of_squares") / ((self.n_groups - 1) * (self.n_genes - 1)) ).alias("normalised_sum_of_squares")
    ...
    def apply_shrinkage(self, intergroup_variance_df, group_mean_variance_df):
        gamma = self.compute_gamma_factor(intergroup_variance_df, group_mean_variance_df)
    ...
    def compute_stability_scoring(self):
        ...
        intragroup_variance_df, gene_means_in_groups_df, dataset_overall_mean = self.get_unbiased_intragroup_variances()

        logger.info("Adjusting variances by group size")
        group_mean_variance_df = self.adjust_for_nb_of_samples_in_groups(intragroup_variance_df)
