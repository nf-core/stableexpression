...
class GeneStatistician:
    ...
    def __post_init__(self):
        ...
        self.samples = self.count_lf.select(pl.exclude(config.ENSEMBL_GENE_ID_COLNAME)).collect_schema().names()
    ...
    def get_samples_with_low_gene_count(self) -> list[str]:
        mean_gene_count = self.gene_count_per_sample_df[config.GENE_COUNT_COLNAME].mean()
        return (
    ...
    def get_main_statistics(self) -> pl.LazyFrame:
        ...
        return augmented_count_lf.select(
            ...
            (pl.col("std") / pl.col("mean")).alias(self.get_colname(config.VARIATION_COEFFICIENT_COLNAME)),
        )
    ...
    def compute_ratios_null_values(self):
        ...
        valid_samples = [sample for sample in self.samples if sample not in self.samples_with_low_gene_count]

        nb_nulls = (
            self.count_lf
            .select(pl.exclude(config.ENSEMBL_GENE_ID_COLNAME).is_null())
            .collect()
            .sum_horizontal()
        )
        nb_nulls_valid_samples = (
            self.count_lf
            .select(pl.col(valid_samples).is_null())
            .collect()
            .sum_horizontal()
        )
        ...
        self.stat_lf = (
            self.stat_lf
            .with_columns(
                ( nb_nulls / len(self.samples) ).alias(self.get_colname(config.RATIO_NULLS_COLNAME)),
                ( nb_nulls_valid_samples / len(valid_samples) ).alias(self.get_colname(config.RATIO_NULLS_VALID_SAMPLES_COLNAME))
            )
        )
    ...
    def compute_ratio_zeros(self):
        nb_zeros = (
            self.count_lf
            .select(pl.exclude(config.ENSEMBL_GENE_ID_COLNAME) == 0)
            .collect()
            .sum_horizontal()
        )
        ...
        self.stat_lf = (
            self.stat_lf
            .with_columns(
                (nb_zeros / len(self.samples)).alias(self.get_colname(config.RATIO_ZEROS_COLNAME)),
            )
        )
    ...
    def get_quantile_intervals(self):
        ...
        self.stat_lf = self.stat_lf.with_columns(
            (pl.col(self.get_colname(config.MEAN_COLNAME)).rank() / pl.col(config.MEAN_COLNAME).count() * self.NB_QUANTILES)
            .floor()
            .cast(pl.Int8)
...
def export_data(
    ...
    logger.info(f"Exporting statistics for all genes to: {ALL_GENES_RESULT_OUTFILENAME}")
    stat_lf.collect().write_csv(ALL_GENES_RESULT_OUTFILENAME)
    logger.info("Done")
